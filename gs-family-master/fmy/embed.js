(function () {
  ...
  var scripts = [
    ...
    'https://www.googletagmanager.com/gtag/js?id=GTM-N9GL44T'
  ...
  loadScripts(scripts, function () {
    initEmbed()
    loadCSS('../../ajax/libs/sweetalert/1.1.3/sweetalert.min.css')
  ...
  function initEmbed() {
    ...
    gtag('config', 'GTM-N9GL44T');
    gtag('get', 'UA-77675003-1', 'client_id', (clientID) => {
    ...
    var btns = document.querySelectorAll('.lp6160135167934464');
    var bgColor = jQuery('.lp6160135167934464').css('background-color')

    for (var i = 0; i < btns.length; i++) {
      btns[i].addEventListener("click", function (e) {
        handler.open({
          name: "gs-family-master",
          email: urlEmail,
          description: validCoupon ? couponDesc : "$97 / yearly",
    ...
    jQuery('.couponLink').click(function (evt) {
      jQuery(evt.target).replaceWith(
        `<form style=`+ formStyle +`  class="couponForm lp6160135167934464">
          <input
            style=` + inputStyle + `
            type="text" 
      ...
      jQuery('.couponForm.lp6160135167934464').submit(function (e) {
        e.preventDefault()
        ...
        jQuery.post(
          ...
          function (data) {
            ...
            if (data.valid) {
                ...
                jQuery(e.target).parent().find('.couponError').text('Coupon successfully applied ✅')
                ...
                var duration = (data.duration === 'forever' || data.duration === 'once')
                    ? data.duration === 'forever' ? 'yearly' : 'one time'
